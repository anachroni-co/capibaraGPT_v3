...
class InteractiveARMVLLM:
    ...
    def print_menu(self):
        ...
        print("5. Estado de carga de modelos")
        print("6. Salir")
        print("-" * 80)
    ...
    def check_model_loading_status(self):
        ...
        self.print_header("ESTADO DE CARGA DE MODELOS")

        try:
            response = requests.get(f"{self.client.server_url}/experts", timeout=10)
            if response.status_code == 200:
                experts_data = response.json()
                
                print("ESTADO DE CARGA DE EXPERTOS:")
                print("-" * 60)
                
                for expert in experts_data['experts']:
                    status = "✅ CARGADO" if expert['is_loaded'] else "⏰ ESPERANDO CARGA"
                    print(f"• {expert['expert_id']}: {status}")
                    print(f"  Dominio: {expert['domain']}, Prioridad: {expert['priority']}")
                    print(f"  Memoria estimada: {expert['estimated_memory_gb']}GB")
                    print()
            else:
    ...
    def run(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.check_model_loading_status()
            elif choice == "6":
