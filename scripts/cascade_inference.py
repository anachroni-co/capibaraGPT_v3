...
def _get_logits(m: LoadedModel, input_ids: np.ndarray) -> np.ndarray:
    ...
    ids = jnp.array(input_ids[np.newaxis, :])
    logits = np.array(m.model.apply(m.params, ids))
    return logits[0, -1, :]
...
def speculative_step(
    ...
    """One speculative decoding step. Returns (new_tokens, n_accepted, n_draft)."""
    ...
    for i, (tok, q_tok) in enumerate(zip(draft_tokens, draft_probs)):
        ...
        if rng.random() < accept_prob:
            ...
            corrected = np.maximum(0.0, target_p_all - q_full)
            s = corrected.sum()
            if s > 1e-10:
                corrected /= s
                tok_new = _sample(corrected, 1.0, rng)
            else:
                tok_new = _sample(target_p_all, temperature, rng)
...
def cascade_generate(
    ...
    generated: list[int] = []
    total_drafted = total_accepted = total_steps = 0
    ...
    stats = {
        ...
        "speedup_estimate": round(toks_per_step, 2),
...
def run_benchmark(
    ...
    for i, prompt in enumerate(prompts):
        ...
        logger.info(
            "  [%2d/%d] α=%.2f  toks/step=%.1f  tok/s=%.0f",
            i + 1, n_samples,
