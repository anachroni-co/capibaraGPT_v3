...
CHUNK_TOKENS = 1024   # tokens per chunk — fits in Slim200M context
MAX_ITERATIONS = 10   # max REPL iterations before fallback
MAX_DEPTH = 3         # max recursive sub-LLM calls
...
def rlm_query(
    ...
    """Process a (potentially very long) document with recursive code generation.

    Args:
        documento: Full document text (any length).
        pregunta: Question to answer.
        inference_fn: fn(system_prompt, user_prompt) -> generated_code string.
        depth: Current recursion depth (0 = top level).

    Returns:
        Answer string, or None if RLM fails (caller should fallback to RAG).
    """
    chunks = _split_document(documento)
    logger.info("RLM depth=%d | chunks=%d | pregunta=%s", depth, len(chunks),
                pregunta[:60])
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        if not ok:
            ...
            if "SyntaxError" in err:
                break  # model can't generate valid Python → fallback
...
def validate_syntax_rate(
    ...
    """Estimate the fraction of model outputs that are syntactically valid Python.

    Target: >70% for RLM to be viable (arXiv:2512.24601v2).
    """
    preguntas = [
...
def main() -> None:
    ...
    parser.add_argument("--validate", action="store_true",
                        help="Run syntax validity test on sample docs")
    parser.add_argument("--sample-docs", default=None,
                        help="Directory with .txt documents for validation")
    parser.add_argument("--n-samples", type=int, default=50)
    parser.add_argument("--documento", default=None, help="Document file to query")
    parser.add_argument("--pregunta", default=None, help="Question to answer")
    parser.add_argument("--model-ckpt", default=None,
                        help="Model checkpoint (uses dummy inference if omitted)")
