...
DOG_BASE = "https://www.dog.xunta.gal"
DOG_SUMARIO = DOG_BASE + "/dog/Publicados/{year}/{month:02d}/{day:02d}/sumario.html"
...
def _dog_issue_urls(year: int, month: int, day: int) -> list[str]:
    ...
    if not html:
        return []
    urls = []
    for match in re.finditer(r'href="(/dog/Publicados/[^"]+\.html)"', html):
        path = match.group(1)
        if "/sumario" not in path:
            urls.append(DOG_BASE + path)
