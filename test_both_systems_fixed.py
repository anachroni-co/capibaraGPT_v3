...
def test_resource_publisher():
    ...
    if resource_data:
        ...
        print(f"   CPU Usage: {resource_data.get('cpu', {}).get('percent', 0)}%")
        print(f"   Memory Usage: {resource_data.get('memory', {}).get('percent', 0)}%")
        print(f"   Disk Usage: {resource_data.get('disk', {}).get('percent', 0)}%")
    else:
