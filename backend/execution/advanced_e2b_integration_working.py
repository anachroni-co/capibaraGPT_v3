...
Sistema completo de gestión de templates y creación dinámica de sandboxes E2B
"""
...
class E2BTemplate:
    """Representa un template de sandbox E2B con configuración predefinida"""
    ...
    def get_sandbox_config(self) -> Dict[str, Any]:
        """Obtiene la configuración para crear un sandbox con este template"""
        return {
...
class AdvancedE2BManager:
    ...
    async def execute_code_with_template(self, 
                                       code: str, 
        ...
        try:
            ...
            # Combinar configuración del template con personalizada
            config = template.get_sandbox_config()
            if custom_config:
                config.update(custom_config)
            
            # Crear sandbox
            logger.info(f"Creando sandbox con template '{template_id}'")
            ...
            # Destruir sandbox después de ejecución
    ...
    async def create_dynamic_sandbox(self, 
        ...
        """
        Crea un sandbox dinámicamente según el tipo de tarea
        """
    ...
    async def execute_on_dynamic_sandbox(self,
                                       code: str,
        ...
        try:
            ...
            # Destruir el sandbox después de la ejecución
            try:
                await sandbox.kill()
                if sandbox_id in self.active_sandboxes:
                    del self.active_sandboxes[sandbox_id]
                ...
                logger.error(f"Error destruyendo sandbox dinámico {sandbox_id}: {e}")
...
class E2BIntegration:
    ...
    async def process_code_request(self, 
        ...
        result['processing_time'] = (datetime.now() - datetime.fromisoformat(result.get('timestamp', datetime.now().isoformat().replace('+00:00', '')))).total_seconds()
...
""",
    ...
    print(f"\n3. Ejecutando con sandbox dinámico (tarea: visualization)...")
